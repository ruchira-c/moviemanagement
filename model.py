...
class Movie:
    ...
    def find_title(title):
        ...
        dsk_movies = dsk_movies.loc[dsk_movies['title'].str.contains(title, flags=re.IGNORECASE)]
        mov_obj = []
        if dsk_movies.empty:
            print("Record does not exist")
            return mov_obj
    ...
    def find_genre(genre):
            ...
            if dsk_movies.empty:
                ...
                for index, row in dsk_movies.iterrows():
                    mov_obj.append(
                        Movie(movieId=row['movieId'], title='', imgpath='', genre='', desc='', avg_ratings=''))
                return mov_obj
